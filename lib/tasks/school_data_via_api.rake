...
task :api_schools => :environment do
  ...
  new_results.each do |school|
    ...
    OfflineSchool.create(bio: description, city: city, ed_level: levels, format: format_description, name: name, site_url: website, state: state, zip_code: zip, street: street, latitude: latitude, longitude: longitude)
  end
